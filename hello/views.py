...
class PlView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({'code': 200, 'message': '评论提交成功，等待审核'})
        except Exception as e:
    ...
    def put(self, request, pk):
        ...
        try:
            ...
            Notify.objects.create(
                ...
                type="comment",
                content=f"你的评论已被{'通过' if status == 'Approved' else '拒绝'}",
                is_read=False
            )
...
class ReplyCommentView(APIView):
    def post(self, request):
        ...
        if not pl_id or not reply:
            return Response({"code": 400, "message": "缺少评论ID或回复内容"})

        try:
            pl = Pl.objects.get(id=pl_id)
            pl.reply = reply
            pl.save()
            return Response({"code": 200, "message": "回复成功"})
        except Pl.DoesNotExist:
